...
public class Main extends Application {
  ...
  public void start(Stage primaryStage) {
    try {
      ...
      GridPane nutritionGrid = new GridPane(); // grid for nutrition informaton
      ...
      EventHandler<ActionEvent> applyFilter = new EventHandler<ActionEvent>() {
        public void handle(ActionEvent e) {
          String filterName = (String) type.getValue(); // Get the values place in the filter field
                                                        // and add them all to the current filters
   ...
   */
  private void addNutritionTableRow(GridPane grid, String food, double cal, double carbs,
      double fat, double fiber, double protein) {
   ...
   */
  private GridPane addFoodToCurrentMealGrid(GridPane mealGrid, FoodItem food,
      GridPane nutritionGrid) {
    int newRow = getRowCount(mealGrid);
    mealGrid.add(new Label(food.getName()), 0, newRow);
    addNutritionTableRow(nutritionGrid, food.getName(),
  ...
  private int getRowCount(GridPane grid) {
    ...
    for (int i = 0; i < grid.getChildren().size(); i++) {
      ...
      if (child.isManaged()) {
        Integer rowIndex1 = GridPane.getRowIndex(child);
        int rowIndex2 = (rowIndex1 != null ? rowIndex1 : 0);
        int rowSpan = getNodeRowSpan(grid, child);
        int rowEnd = (rowSpan != GridPane.REMAINING ? getNodeRowIndex(grid, child) + rowSpan - 1
            : GridPane.REMAINING);
  ...
  /**
   * Returns the number of rows that node spans inside of the grid object. If it cannot be found, it
   * returns 1
   * 
   ...
   * @return button - the remove button that was added
   */
  ...
  private static void removeRow(GridPane grid, int rowToRemove) {
    // set to gather all nodes in the row we want toremove
