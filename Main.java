...
/**
 * This class is the main backbone of the food query/meal analysis program. It sets up all the data
 * structures needed to add food items to a list, create meals, analyze meals, and filter food
 * items.
 * 
...
public class Main extends Application {
  ...
  public void start(Stage primaryStage) {
    try {
      ...
      filteredFoods = foodList.getAllFoodItems(); // Set the filtered list to the full list
                                                  // initially
      updateListLabel(foodCountLbl, filteredFoods);

      // Scene will consist of a VBox. The top node of the box will just be the exit button, and the
      // bottom object will be a split pane
      VBox root = new VBox();
      ...
      type.focusedProperty().addListener((ov, oldValue, newValue) -> {
        if ((newValue != null) && (operator.getValue() != null) && (!tf3.getText().isEmpty())) {
      ...
      operator.focusedProperty().addListener((ov, oldValue, newValue) -> {
        if ((newValue != null) && (type.getValue() != null) && (!tf3.getText().isEmpty())) {
          addFilter.setDisable(false);
        } else {
          addFilter.setDisable(true);
        }
      ...
      tf3.focusedProperty().addListener((ov, oldValue, newValue) -> {
        if ((newValue != null) && (type.getValue() != null) && (operator.getValue() != null)) {
          addFilter.setDisable(false);
        } else {
          addFilter.setDisable(true);
        }
      ...
      EventHandler<ActionEvent> applyFilter = new EventHandler<ActionEvent>() {
        public void handle(ActionEvent e) {
          String filterName = null; // Get the values place in the filter field and add them all to
                                    // the current filters
          ...
          try {
            ...
            // don't allow name filter for non-contains operator, similarly don't allow contains to
            // be used with non-Name filter
            ...
            fail.setContentText(
                "Filter value for " + operatorName + " operator must be a positive number");
            fail.setHeaderText(null);
            fail.setGraphic(null);
            type.valueProperty().set(null);
            operator.valueProperty().set(null);
          ...
          filterVBox.getChildren().addAll(addFilt, filters); // Add all the labels back to the
                                                             // filter section so it appear
                                                             // appropriately
          filterVBox.getChildren().addAll(appliedFilters);
          filterVBox.getChildren().addAll(removeFilt);
          List<FoodItem> tempList = new ArrayList<FoodItem>();
          if (filterName == "Name" && valueName != null) {
            tempList = foodList.filterByName(valueName); // If the user chooses to filter on name,
                                                         // use that to find our temporary list
              ...
              public void handle(ActionEvent event) {
                if (foodListView.getSelectionModel().getSelectedItem() != null) {
                  ...
                  for (i = 0; i < meal.getMealList().size(); i++) {
                    if (meal.getMealList().get(i).getName()
                        .compareToIgnoreCase(addFood.getName()) > 0) {
                      break;
                    }
                  }
                  ...
                  removeButton.setOnAction(new EventHandler<ActionEvent>() {
                    ...
                    public void handle(ActionEvent event) {
                      ...
                      meal.removeFromMeal(
                          meal.getMealList().get(GridPane.getRowIndex(removeButton)));
                      ...
                      // update totals row
                      totalCalories
                          .setText(Double.toString(meal.getTotalNutrient(Nutrients.CALORIES)));
                      totalCarbs
                          .setText(Double.toString(meal.getTotalNutrient(Nutrients.CARBOHYDRATES)));
                      ...
                      totalProtein
                          .setText(Double.toString(meal.getTotalNutrient(Nutrients.PROTEIN)));
                    }
          ...
          saveFood.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
              ...
              try {
                ...
                if (!(calsD >= 0.0) | !(carbsD >= 0.0) | !(fatD >= 0.0) | !(fiberD >= 0.0)
                    | !(proteinD >= 0.0)) {
                  Alert fail = new Alert(AlertType.ERROR);
                  fail.setTitle("Invalid Nutrient");
                  fail.setContentText(
                      "Nutrient values must be a number greater than or equal to zero");
                  fail.setHeaderText(null);
                  fail.setGraphic(null);
                  calories.clear();
                  carbs.clear();
                ...
                String id = "" + nameHash + calsD.toString().replace(".", "")
                    + carbsD.toString().replace(".", "") + fatD.toString().replace(".", "")
                    + fiberD.toString().replace(".", "") + proteinD.toString().replace(".", "");
                ...
                try {
                  ...
                  appliedFilters.clear(); // Remove all the filters from the list
                  filterVBox.getChildren().clear();
                  filterVBox.getChildren().addAll(addFilt, filters); // Add all the labels back to
                                                                     // the filter section so it
                  ...
                  filteredFoods = foodList.getAllFoodItems(); // Reset the filtered list to
                                                              // everything
                  updateListLabel(foodCountLbl, filteredFoods);
      ...
      addAndSaveH.getChildren().addAll(addFood, addAndSaveRegion1, foodCountLbl, addAndSaveRegion2,
          saveList);
      ...
      EventHandler<ActionEvent> loadFileEvent = new EventHandler<ActionEvent>() {
        public void handle(ActionEvent e) {
          ...
          if (newFoodList != null) {
            ...
            filteredFoods = foodList.getAllFoodItems(); // Reset the filtered list to the new food
                                                        // items
            appliedFilters.clear(); // Remove all the filters from the list
            filterVBox.getChildren().clear();
            filterVBox.getChildren().addAll(addFilt, filters); // Add all the labels back to the
      ...
      right.getChildren().addAll(currentMeal, mealScrollPane, region5, mealNutrition,
          nutritionScrollPane);
      right.setSpacing(25);
   ...
   */
  private void addNutritionTableRow(int index, GridPane grid, String food, double cal, double carbs,
      double fat, double fiber, double protein) {// , boolean isBold) {
   ...
   * @param nutritionGrid - the nutrition grid of the bottom of the screen that will also be updated
   *        with this food's nutrition info
   * @return the mealGrid GridPane after the food has been added
   */
  private void addFoodToCurrentMealGrid(int index, GridPane mealGrid, FoodItem food,
      GridPane nutritionGrid) {
    // bump down lower rows
    ...
    addNutritionTableRow(index + 1, nutritionGrid, food.getName(),
        food.getNutrientValue(Nutrients.CALORIES.toString()),
        food.getNutrientValue(Nutrients.CARBOHYDRATES.toString()),
        food.getNutrientValue(Nutrients.FAT.toString()),
        food.getNutrientValue(Nutrients.FIBER.toString()),
        food.getNutrientValue(Nutrients.PROTEIN.toString())); // , false);
  }
  ...
  private void setUpNutritionGrid(GridPane nutritionGrid) {
    ...
    nutritionGrid.getColumnConstraints().addAll(col1, otherCols, otherCols, otherCols, otherCols,
        otherCols);
  ...
  /***
   * Updates the label below the food list with how many items are in the list. Updated whenever a
   * food is added or a new label is applied
   * 
  ...
  private static void updateListLabel(Label countFoods, List<FoodItem> filteredList) {
    ...
    switch (foodsInList) {
      case 0:
        numFoods = "There are no items in the list";
        break;
      case 1:
        numFoods = "There is 1 item in the list";
        break;
      default:
        numFoods = "There are " + foodsInList + " items in the list";
        break;
    }
  ...
  public static void main(String[] args) {
    ...
    // instantiate an ADT
    /*
     * parseFoodList("foodItems.txt"); for (FoodItem food : foodList) {
